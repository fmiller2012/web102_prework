...
function addGamesToPage(games) 
{
    ...
    for(let i = 0; i < games.length; i++)
    {
        ...
        //append the game to games-container
...
const totalContributions = GAMES_JSON.reduce((contributions, contribution) => {
	return contributions + contribution.backers;
}, 0);
...
const totalGames = GAMES_JSON.reduce((total, contribution) => {
	return total + 1;
}, 0); 
...
function filterUnfundedOnly() {
    ...
    let listOfDecreasedRaised = GAMES_JSON.filter( (game) => {
	return game.pledged < game.goal;
    });

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(listOfDecreasedRaised);
    console.log(listOfDecreasedRaised)
}
filterUnfundedOnly();
...
function filterFundedOnly() {
    ...
    let listMetExceededGames = GAMES_JSON.filter( (game) => {
	return game.pledged >= game.goal;
    });

    // use the function we previously created to add unfunded games to the DOM
    addGamesToPage(listMetExceededGames);
    console.log(listMetExceededGames)
}
filterFundedOnly();
...
{
	countUnfunded = GAMES_JSON[i].pledged < GAMES_JSON[i].goal ? countUnfunded++ : countUnfunded;
}
...
const explainStr = `
A total of $${totalPledged.toLocaleString()} has been raised for ${totalGames2} ${totalGames2 === 1 ? 'game' : 'games'}.
